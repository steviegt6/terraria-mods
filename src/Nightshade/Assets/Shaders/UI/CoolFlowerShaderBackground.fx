...
void frag_main()
{
    ...
    // CHANGE: Add rotation over time.
    // float3 r = normalize(float3(st, 1.0f));
    float angleX = 0.2 * iTime;
    // float2 rotatedSt = float2x2(cos(angleX), -sin(angleX), sin(angleX), cos(angleX)) * st;
    float2x2 rotX = float2x2(cos(angleX), -sin(angleX), sin(angleX), cos(angleX));
    float2 rotatedSt = mul(rotX, st);
    float3 r = normalize(float3(rotatedSt, 1.0));
