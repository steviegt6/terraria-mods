...
internal class FlailChainTweak : GlobalProjectile
{
    ...
    public VerletIntegratedBodyHandler? Handler { get; set; } = null;

    [InitializedInLoad]
    private static WrapperShaderData<Assets.Shaders.Misc.BasicPixelizationShader.Parameters>? shader;
    ...
    public override void Load()
    {
        ...
        Main.QueueMainThreadAction(() =>
            {
                shader = Assets.Shaders.Misc.BasicPixelizationShader.CreateStripShader();
                {
        ...
        On_Main.DrawProj_FlailChains += (orig, projectile, mountedCenter) =>
        {
            ...
            );
            {
                ...
                shader.Parameters.uSize = new Vector2(managedRenderTarget.Value.Width, managedRenderTarget.Value.Height);
                shader.Apply();
